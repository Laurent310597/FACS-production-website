...
export default function AdminFormEmailPage() {
  ...
  return (
    <AdminLayout>
      ...
      <section className="mt-6 rounded-[28px] border border-white/10 bg-white/[0.035] p-6 md:p-8"><h2 className="text-xl font-semibold">Kiểm tra địa chỉ gửi</h2><p className="mt-2 text-sm text-slate-500">Sau khi kết nối, gửi thử từng alias và kiểm tra email nhận được hiển thị đúng trường From.</p><div className="mt-5 grid gap-4 md:grid-cols-2">{aliases.map((alias) => <div key={alias.email} className="rounded-2xl border border-white/10 bg-[#081321]/60 p-5"><div className="font-semibold">{alias.name}</div><div className="mt-1 text-sm text-cyan-200">{alias.email}</div><button disabled={!connection.connected || working === alias.email} onClick={() => testAlias(alias.email)} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-white/10 px-4 py-2.5 text-sm font-semibold disabled:opacity-40">{working === alias.email ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />} Gửi thử</button></div>)}</div></section>
    </AdminLayout>
  );
}
