...
import { CheckCircle2, Eye, Loader2, MailCheck, RefreshCw, RotateCcw, Save, Send, X, XCircle } from "lucide-react";
...
export default function AdminFormEmailPage() {
  ...
  const loadConnection = async () => {
    try {
      ...
      return `Microsoft 365: ${loadError.message}`;
    }
  };
  ...
  useEffect(() => {
    ...
    Promise.allSettled([invokeFormEmailAdmin("oauth_status"), invokeFormEmailAdmin("get_templates")]).then(([statusResult, templatesResult]) => {
      ...
      else { setConnection({ loading: false, connected: false }); issues.push(`Microsoft 365: ${statusResult.reason.message}`); }
  ...
  return (
    <AdminLayout>
      ...
      <section className="mt-7 rounded-[28px] border border-white/10 bg-white/[0.035] p-6 md:p-8"><div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between"><div><div className="flex items-center gap-3"><MailCheck className="text-cyan-300" /><h2 className="text-xl font-semibold">Microsoft 365</h2></div><div className="mt-4 flex items-center gap-2 text-sm">{connection.loading ? <><Loader2 size={17} className="animate-spin" /> Đang kiểm tra...</> : connection.connected ? <><CheckCircle2 size={18} className="text-emerald-300" /><span className="text-emerald-200">Đã cấu hình gửi email Outlook</span></> : <><XCircle size={18} className="text-amber-300" /><span className="text-amber-200">Chưa cấu hình Microsoft Graph</span></>}</div><p className="mt-2 text-xs text-slate-500">Hệ thống giữ nguyên các mailbox FACS và xác thực an toàn bằng ứng dụng Microsoft 365.</p></div><button onClick={load} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-4 py-3 text-sm"><RefreshCw size={17} /> Kiểm tra</button></div></section>
